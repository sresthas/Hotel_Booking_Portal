...
public class Taj extends JFrame {
	...
	public Taj(final String person,final String checkin,final String checkout,final String rooms,final long days) {
		...
		/*Scanner ss=new Scanner("Taj.txt");
		int y;
		for(int i=0;i<30;i++)
		{
			String x=ss.next();
		...
		try {
			...
			String sql = "SELECT Rooms FROM TajN ";
			...
			String sql1 = "SELECT Rooms FROM TajD ";
		...
		final int price=(int) (days*4500*roo);
			...
			public void actionPerformed(ActionEvent e) {
				...
				*/
				...
				try {
					...
					int y=i;
					String query="Update TajN set Rooms='"+x+"' where Days='"+y+"'";
				...
				try
				{
					...
					////System.out.println("Day"+i+"="+x);
					int y=i;
					String query="Update TajD set Rooms='"+x+"' where Days='"+y+"'";
					PreparedStatement pst=connection.prepareStatement(query);
					pst.execute();
					pst.close();
					}
				...
				try {
					String query="Update user_data set Hotel='Taj' where username='"+person+"'";
					PreparedStatement pst=connection.prepareStatement(query);
					pst.execute();
					pst.close();
					//connection.close();
