...
public class Hyatt extends JFrame {
	...
	public Hyatt(final String person,final String checkin,final String checkout,final String rooms,final long days) {
		...
		/*Scanner ss=new Scanner("Hyatt.txt");
		int y;
		for(int i=0;i<30;i++)
		{
			String x=ss.next();
		...
		try {
			...
			String sql = "SELECT Rooms FROM HyattN ";
			...
			String sql1 = "SELECT Rooms FROM HyattD ";
		...
		final int price=(int) (days*12000*roo);
			...
			public void actionPerformed(ActionEvent e) {
				...
				*/
				...
				try {
					...
					int y=i;
					String query="Update HyattN set Rooms='"+x+"' where Days='"+y+"'";
				...
				try
				{
					...
					////System.out.println("Day"+i+"="+x);
					int y=i;
					String query="Update HyattD set Rooms='"+x+"' where Days='"+y+"'";
					PreparedStatement pst=connection.prepareStatement(query);
					pst.execute();
					pst.close();
					//connection.close();
				...
				try {
					String query="Update user_data set Hotel='Hyatt' where username='"+person+"'";
					PreparedStatement pst=connection.prepareStatement(query);
					pst.execute();
					pst.close();
					//connection.close();
