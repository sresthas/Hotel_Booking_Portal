...
public class Hilton extends JFrame {
	...
	public Hilton(final String person,final String checkin,final String checkout,final String rooms,final long days) {
		...
		/*Scanner ss=new Scanner("Hilton.txt");
		int y;
		for(int i=0;i<30;i++)
		{
			String x=ss.next();
		...
		try {
			...
			String sql = "SELECT Rooms FROM HiltonN ";
			...
			String sql1 = "SELECT Rooms FROM HiltonD ";
		...
		final int price=(int) (days*5200*roo);
			...
			public void actionPerformed(ActionEvent e) {
				...
				*/
				...
				try {
					...
					int y=i;
					String query="Update HiltonN set Rooms='"+x+"' where Days='"+y+"'";
				...
				try
				{
					...
					////System.out.println("Day"+i+"="+x);
					int y=i;
					String query="Update HiltonD set Rooms='"+x+"' where Days='"+y+"'";
					PreparedStatement pst=connection.prepareStatement(query);
					pst.execute();
					pst.close();
					}
				...
				try {
					String query="Update user_data set Hotel='Hilton' where username='"+person+"'";
					PreparedStatement pst=connection.prepareStatement(query);
					pst.execute();
					pst.close();
					//connection.close();
